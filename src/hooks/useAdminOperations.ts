  ...
  const updateFirm = async (id: string, updates: Partial<PropFirm>) => {
    ...
    try {
      ...
      const formattedUpdates = {
        ...
        table_price: table_price !== undefined ? table_price : undefined,
        table_profit_split: table_profit_split !== undefined ? table_profit_split : undefined,
        table_payout_rate: table_payout_rate !== undefined ? table_payout_rate : undefined,
        table_platform: table_platform !== undefined ? table_platform : undefined,
        table_trust_rating: table_trust_rating !== undefined ? table_trust_rating : undefined,
        table_evaluation_rules: table_evaluation_rules !== undefined ? table_evaluation_rules : undefined,
        table_fee: table_fee !== undefined ? table_fee : undefined,
        table_coupon_code: table_coupon_code !== undefined ? table_coupon_code : undefined,
      };
      ...
      if (show_in_table_review !== undefined || table_review_priority !== undefined) {
        const { error: tableError } = await supabase<dyad-command type="refresh"></dyad-command><dyad-write path="src/hooks/useAdminOperations.ts" description="Updating useAdminOperations to handle table review data">
import { useState } from 'react';
import { supabase } from '@/integrations/supabase/client';
import { PropFirm } from '@/types/supabase';
import { useToast } from '@/hooks/use-toast';
